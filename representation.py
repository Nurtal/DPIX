...
def get_neighbour(var, grid_matrix, half_size_block):
	...
	if(y_position >= half_size_block and x_position >= half_size_block):
		for x in xrange(0,-half_size_block):
			n_x_position = x
			for y in xrange(0,-half_size_block):
				n_y_position = y
...
def score_converger(grid_matrix, distance_matrix):
    ...
    ## parameters
    number_of_candidate = int(len(grid_matrix)/4)
...
def build_image_map_GMA(scaled_data_file, nb_iteration):
    """
